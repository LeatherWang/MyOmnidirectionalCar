...
int RC_Velocity=30,RC_Position=3000,RCX_Position,RCY_Position,RCZ_Position;   //设置遥控的速度和位置值
u8 Usart3_Receive,Left_Receive,Right_Receive,Run_Flag=0;  //蓝牙遥控相关变量和运行状态标志位
u8 rxbuf[8],Urxbuf[8],CAN_ON_Flag=0,Usart_ON_Flag=0,Usart_Flag;  //CAN和串口控制相关变量
...
{ 
	...
	KEY_Init();                     //=====按键初始化
	if(MODE==0)Run_Flag=1;           //=====启动的过程中，用户按键被按下，即可进入位置默认
	else Run_Flag=0;                //=====否则就是默认的速度模式
	OLED_Init();                    //=====OLED初始化
